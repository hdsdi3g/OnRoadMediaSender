...
public class ExecprocessPipedCascade {
	...
	public void setWorkingDirectory(File working_directory) throws IOException {
		FileValidation.checkExistsCanRead(working_directory);
		FileValidation.checkIsDirectory(working_directory);
		this.working_directory = working_directory;
		list_process.forEach(v -> {
			try {
				v.setWorkingDirectory(working_directory);
			} catch (Exception e) {
				log.warn("Can't set working_directory " + working_directory); //$NON-NLS-1$
			}
		});
	}
	
	public void add(File execname, ArrayList<String> params) {
		Execprocess new_item = new Execprocess(execname, params, new Event(execname.getName() + "/" + list_process.size())); //$NON-NLS-1$
		...
		new_item.setName("Execproces for " + execname.getName() + ", pipe #" + list_process.size()); //$NON-NLS-1$ //$NON-NLS-2$
	...
	void connectSourceDatas(Execprocess ref, OutputStream process_std_in) {
		log.debug("Set pipe next_process_std_in by " + ref.getExec_name()); //$NON-NLS-1$
	...
	void connectDestDatas(Execprocess ref, InputStream process_std_out) {
		log.debug("Set pipe previous_process_std_out by " + ref.getExec_name()); //$NON-NLS-1$
	...
	private class Pipe extends Thread {
		...
		public Pipe(String previous_process, String next_process_name, int pipe_num) {
			setDaemon(true);
			setName("Pipe process for " + previous_process + " > " + next_process_name + ", #" + pipe_num); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		}
		
		public void run() {
			log.debug("Start " + getName()); //$NON-NLS-1$
			
			try {
				IOUtils.copyLarge(previous_process_std_out, next_process_std_in);
			} catch (IOException e) {
				if (e.getMessage().equalsIgnoreCase("Broken pipe")) { //$NON-NLS-1$
				} else {
					log.error("With streams", e); //$NON-NLS-1$
				}
			}
			try {
			...
			log.trace("End " + getName()); //$NON-NLS-1$
		}
	...
	private class Event implements ExecprocessEvent {
		...
		public Event(String name) {
			this.name = name;
			if (name == null) {
				throw new NullPointerException("\"name\" can't to be null"); //$NON-NLS-1$
			}
		}
		
		public void onStart(String commandline, File working_directory) {
			log.info("Start process " + name + ": " + commandline); //$NON-NLS-1$ //$NON-NLS-2$
		}
		
		public void onEnd(int exitvalue, long execution_duration) {
			log.debug("End process " + name + ", return " + exitvalue + " during " + (double) execution_duration / 1000d + " sec"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		}
		
		public void onKill(long execution_duration) {
			log.debug("Kill process " + name + ", after " + (double) execution_duration / 1000d + " sec"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		}
		
		public void onError(IOException ioe) {
			log.error("Process " + name + " has an error ", ioe); //$NON-NLS-1$ //$NON-NLS-2$
		}
		
		public void onError(InterruptedException ie) {
			log.warn("Process " + name + " has an error ", ie); //$NON-NLS-1$ //$NON-NLS-2$
		}
		
		public void onStdout(String message) {
			log.debug("Process " + name + " stdout: " + message); //$NON-NLS-1$ //$NON-NLS-2$
		}
		
		public void onStderr(String message) {
			if (log.isTraceEnabled()) {
				System.err.println("[" + name + "]" + message); //$NON-NLS-1$ //$NON-NLS-2$
			}
		}
